var langs = [
    {
        "language": "af",
        "name": "Afrikaans",
        "score": 0.675
    },
    {
        "language": "sq",
        "name": "Albanian",
        "score": 0.2625
    },
    {
        "language": "am",
        "name": "Amharic",
        "score": 0.3
    },
    {
        "language": "ar",
        "name": "Arabic",
        "score": 0.325
    },
    {
        "language": "hy",
        "name": "Armenian",
        "score": 0.25
    },
    {
        "language": "az",
        "name": "Azerbaijani",
        "score": 0.375
    },
    {
        "language": "eu",
        "name": "Basque",
        "score": 0.375
    },
    {
        "language": "be",
        "name": "Belarusian",
        "score": 0.4
    },
    {
        "language": "bn",
        "name": "Bengali",
        "score": 0.01
    },
    {
        "language": "bs",
        "name": "Bosnian",
        "score": 0.3
    },
    {
        "language": "bg",
        "name": "Bulgarian",
        "score": 0.4
    },
    {
        "language": "ca",
        "name": "Catalan",
        "score": 0.375
    },
    {
        "language": "ceb",
        "name": "Cebuano",
        "score": 0.125
    },
    {
        "language": "ny",
        "name": "Chichewa",
        "score": 0.175
    },
    {
        "language": "zh",
        "name": "Chinese (Simplified)",
        "score": 0.55
    },
    {
        "language": "zh-TW",
        "name": "Chinese (Traditional)",
        "score": 0.55
    },
    {
        "language": "co",
        "name": "Corsican",
        "score": 0.225
    },
    {
        "language": "hr",
        "name": "Croatian",
        "score": 0.55
    },
    {
        "language": "cs",
        "name": "Czech",
        "score": 0.4375
    },
    {
        "language": "da",
        "name": "Danish",
        "score": 0.4
    },
    {
        "language": "nl",
        "name": "Dutch",
        "score": 0.525
    },
    {
        "language": "en",
        "name": "English",
        "score": 1
    },
    {
        "language": "eo",
        "name": "Esperanto",
        "score": 0.4
    },
    {
        "language": "et",
        "name": "Estonian",
        "score": 0.275
    },
    {
        "language": "tl",
        "name": "Filipino",
        "score": 0.25
    },
    {
        "language": "fi",
        "name": "Finnish",
        "score": 0.45
    },
    {
        "language": "fr",
        "name": "French",
        "score": 0.45
    },
    {
        "language": "fy",
        "name": "Frisian",
        "score": 0.3
    },
    {
        "language": "gl",
        "name": "Galician",
        "score": 0.525
    },
    {
        "language": "ka",
        "name": "Georgian",
        "score": 0.1
    },
    {
        "language": "de",
        "name": "German",
        "score": 0.6
    },
    {
        "language": "el",
        "name": "Greek",
        "score": 0.525
    },
    {
        "language": "gu",
        "name": "Gujarati",
        "score": 0.3
    },
    {
        "language": "ht",
        "name": "Haitian Creole",
        "score": 0.01
    },
    {
        "language": "ha",
        "name": "Hausa",
        "score": 0.275
    },
    {
        "language": "haw",
        "name": "Hawaiian",
        "score": 0.15
    },
    {
        "language": "iw",
        "name": "Hebrew",
        "score": 0.45
    },
    {
        "language": "hi",
        "name": "Hindi",
        "score": 0.225
    },
    {
        "language": "hmn",
        "name": "Hmong",
        "score": 0.275
    },
    {
        "language": "hu",
        "name": "Hungarian",
        "score": 0.475
    },
    {
        "language": "is",
        "name": "Icelandic",
        "score": 0.2
    },
    {
        "language": "ig",
        "name": "Igbo",
        "score": 0.475
    },
    {
        "language": "id",
        "name": "Indonesian",
        "score": 0.475
    },
    {
        "language": "ga",
        "name": "Irish",
        "score": 0.25
    },
    {
        "language": "it",
        "name": "Italian",
        "score": 0.5
    },
    {
        "language": "ja",
        "name": "Japanese",
        "score": 0.425
    },
    {
        "language": "jw",
        "name": "Javanese",
        "score": 0.175
    },
    {
        "language": "kn",
        "name": "Kannada",
        "score": 0.2
    },
    {
        "language": "kk",
        "name": "Kazakh",
        "score": 0.425
    },
    {
        "language": "km",
        "name": "Khmer",
        "score": 0.3
    },
    {
        "language": "ko",
        "name": "Korean",
        "score": 0.425
    },
    {
        "language": "ku",
        "name": "Kurdish (Kurmanji)",
        "score": 0.0375
    },
    {
        "language": "ky",
        "name": "Kyrgyz",
        "score": 0.225
    },
    {
        "language": "lo",
        "name": "Lao",
        "score": 0.2125
    },
    {
        "language": "la",
        "name": "Latin",
        "score": 0.05
    },
    {
        "language": "lv",
        "name": "Latvian",
        "score": 0.5
    },
    {
        "language": "lt",
        "name": "Lithuanian",
        "score": 0.2
    },
    {
        "language": "lb",
        "name": "Luxembourgish",
        "score": 0.275
    },
    {
        "language": "mk",
        "name": "Macedonian",
        "score": 0.35
    },
    {
        "language": "mg",
        "name": "Malagasy",
        "score": 0.275
    },
    {
        "language": "ms",
        "name": "Malay",
        "score": 0.05
    },
    {
        "language": "ml",
        "name": "Malayalam",
        "score": 0.425
    },
    {
        "language": "mt",
        "name": "Maltese",
        "score": 0.35
    },
    {
        "language": "mi",
        "name": "Maori",
        "score": 0.1
    },
    {
        "language": "mr",
        "name": "Marathi",
        "score": 0.15
    },
    {
        "language": "mn",
        "name": "Mongolian",
        "score": 0.4
    },
    {
        "language": "my",
        "name": "Myanmar (Burmese)",
        "score": 0.125
    },
    {
        "language": "ne",
        "name": "Nepali",
        "score": 0.025
    },
    {
        "language": "no",
        "name": "Norwegian",
        "score": 0.275
    },
    {
        "language": "ps",
        "name": "Pashto",
        "score": 0.2
    },
    {
        "language": "fa",
        "name": "Persian",
        "score": 0.1
    },
    {
        "language": "pl",
        "name": "Polish",
        "score": 0.5625
    },
    {
        "language": "pt",
        "name": "Portuguese",
        "score": 0.525
    },
    {
        "language": "pa",
        "name": "Punjabi",
        "score": 0.1
    },
    {
        "language": "ro",
        "name": "Romanian",
        "score": 0.425
    },
    {
        "language": "ru",
        "name": "Russian",
        "score": 0.4
    },
    {
        "language": "sm",
        "name": "Samoan",
        "score": 0.15
    },
    {
        "language": "gd",
        "name": "Scots Gaelic",
        "score": 0.4
    },
    {
        "language": "sr",
        "name": "Serbian",
        "score": 0.475
    },
    {
        "language": "st",
        "name": "Sesotho",
        "score": 0.325
    },
    {
        "language": "sn",
        "name": "Shona",
        "score": 0.175
    },
    {
        "language": "sd",
        "name": "Sindhi",
        "score": 0.3
    },
    {
        "language": "si",
        "name": "Sinhala",
        "score": 0.35
    },
    {
        "language": "sk",
        "name": "Slovak",
        "score": 0.275
    },
    {
        "language": "sl",
        "name": "Slovenian",
        "score": 0.4
    },
    {
        "language": "so",
        "name": "Somali",
        "score": 0.15
    },
    {
        "language": "es",
        "name": "Spanish",
        "score": 0.575
    },
    {
        "language": "su",
        "name": "Sundanese",
        "score": 0.225
    },
    {
        "language": "sw",
        "name": "Swahili",
        "score": 0.15
    },
    {
        "language": "sv",
        "name": "Swedish",
        "score": 0.45
    },
    {
        "language": "tg",
        "name": "Tajik",
        "score": 0.01
    },
    {
        "language": "ta",
        "name": "Tamil",
        "score": 0.2
    },
    {
        "language": "te",
        "name": "Telugu",
        "score": 0.225
    },
    {
        "language": "th",
        "name": "Thai",
        "score": 0.15
    },
    {
        "language": "tr",
        "name": "Turkish",
        "score": 0.4
    },
    {
        "language": "uk",
        "name": "Ukrainian",
        "score": 0.3
    },
    {
        "language": "ur",
        "name": "Urdu",
        "score": 0.075
    },
    {
        "language": "uz",
        "name": "Uzbek",
        "score": 0.175
    },
    {
        "language": "vi",
        "name": "Vietnamese",
        "score": 0.2
    },
    {
        "language": "cy",
        "name": "Welsh",
        "score": 0.4375
    },
    {
        "language": "xh",
        "name": "Xhosa",
        "score": 0.1125
    },
    {
        "language": "yi",
        "name": "Yiddish",
        "score": 0.275
    },
    {
        "language": "yo",
        "name": "Yoruba",
        "score": 0.175
    },
    {
        "language": "zu",
        "name": "Zulu",
        "score": 0.175
    }
];

/**
 * Returns the ISO 639-1 code of the desiredLang – if it is supported by Google Translate
 * @param {string} desiredLang – the name or the code of the desired language
 * @returns {string|boolean} The ISO 639-1 code of the language or false if the language is not supported
 */
function getCode(desiredLang) {
    desiredLang = desiredLang.toString().toLowerCase();

    for (var i = 0; i < langs.length; i++) {
        if (langs[i].name.toLowerCase() == desiredLang)
            return langs[i].language
    }
    return false
}

/**
 * Returns the weight of the desiredLang – if it is supported by Google Translate
 * @param {string} desiredLang – the name or the code of the desired language
 * @returns {string|boolean} The weight of the language or false if the language is not supported
 */
function getWeight(desiredLang) {
    if (!desiredLang) {
        return false;
    }
    desiredLang = desiredLang.toLowerCase();

    if (langs[desiredLang]) {
        return desiredLang;
    }

    for (var i = 0; i < langs.length; i++) {
        if (langs[i].name.toLowerCase() == desiredLang)
            return langs[i].score
    }
    return false
}

function getLanguageNames() {

    var newObject = {};
    for (var i = 0; i < langs.length; i++) {
        newObject[langs[i].name] = null
    }
    return newObject
}

module.exports = langs;
module.exports.getCode = getCode;
module.exports.getWeight = getWeight;
module.exports.getLanguageNames = getLanguageNames;
